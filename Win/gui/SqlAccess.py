...
class MySqlConnect:
    ...
    def __init__(self, local_bind_p):
        ...
        mysql = config['mysql.access.opendarkness']
        self.local_bind_p = local_bind_p
        self.connection = connect(host=mysql['host'], port=self.local_bind_p, user=mysql['user'],
                                  passwd=mysql['passwd'],
                                  db=mysql['db'], charset=mysql['charset'], use_unicode=True)
    ...
    def getprojid(self, accid):
        ...
        with self.connection:
            cursor = self.connection.cursor(cursors.DictCursor)
            sqlex = ("SELECT project_id FROM Project WHERE account_id= " + str(
                accid) + " AND project_inactive= 0 " + "ORDER BY project_id ASC")
            cursor.execute(sqlex)
            return cursor.fetchall()

    def deleteproject(self, project_id):
        """
        Deletes NOT the project itself, it just puts it to inactive state.
        Inserts a "1" for signaling it is inactive.
        :param project_id:
        :return:
        """
        with self.connection:
    ...
    def setproject(self, proj_name, proj_descr, acc_id):
        """
        Enter the Project Data into the Database for new Projects.
        :param proj_name: string
