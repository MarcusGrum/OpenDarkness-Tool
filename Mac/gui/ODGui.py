...
import matplotlib       # MUST for Mac
matplotlib.use('TkAgg') # MUST for Mac
...
class ODGui:
    ...
    def header(self):
        ...
        logopath = resource_path("gui/images/OD_logo_long.gif")
    ...
    def startscreen(self):
        ...
        self.windows["startscreen"].pack(side='bottom', fill='both', expand=1)
        logopath = resource_path("gui/images/big_start_logo.gif")
    ...
    def projectdata(self):
        ...
        # Logo Frame
        logopath = resource_path("gui/images/OD_logo_small.gif")
        load = Image.open(logopath)
        logo_od = ImageTk.PhotoImage(load)
    ...
    def connecttoserver(self):
        ...
        if self.connectionstatus == 0:
            ...
            address = self.config.get('sshtunnel.opendarkness', 'server.address')
            port = self.config.getint('sshtunnel.opendarkness', 'server.port')
            pwd = self.config.get('sshtunnel.opendarkness', 'ssh_password')
            user = self.config.get('sshtunnel.opendarkness', 'ssh_username')
            remaddy = self.config.get('sshtunnel.opendarkness', 'remote_bind_address.address')
            remport = self.config.getint('sshtunnel.opendarkness', 'remote_bind_address.port')

            try:
                self.server = SSHTunnelForwarder((address, port),
                                                 ssh_password=pwd,
                                                 ssh_username=user,
                                                 remote_bind_address=(remaddy,
                                                                      remport))
                self.server.start()
    ...
    def createproject(self):
        ...
        if self.loginstatus == 0:
            ...
            self.entries["description"] = Text(self.windows["createframe"], width=44, height=4, wrap=WORD,
                                               cursor='xterm', relief='groove', borderwidth=2)  # tk.WORD
    ...
    def deleteproject(self, proj_id):
        """
        Deletes the whole project from account. Leaves the data of the qustionaires.
        :param proj_id:
        :return:
        """
        ...
        if askyesno("Projekt löschen!",
                    ...
                    "Wenn Sie diese tun gehen alle Daten verloren.\nSind Sie sicher?"):
    ...
    def infowindow(self):
        ...
        Opens the interent browser by clicking on link.
        :return:
        """
    ...
    def partieswindow(self):
        ...
        logopath = resource_path("gui/images/parties.gif")
        logoparty = ImageTk.PhotoImage(Image.open(logopath))
    ...
    def openmanual(self):
        ...
        call(['open', manualpath])
        # print "1"
        # manualpath = resource_path("usedlicences.txt")
        # print("2")
        # open_cmd = "open " + manualpath
        # print("3")
        # system(open_cmd)
        # print("4")
